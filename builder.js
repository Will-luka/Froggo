...
function gerarBarrasEmpilhadas(statsBase, statsItens, itensSelecionados) {
    ...
    nomesStats.forEach((nomeStat, index) => {
        ...
        if (base > 0) {
            ...
            segmentos += `
                <div class="stat-bar-segment" style="width:${largura}%; background-color:${coresBase};" data-tooltip="Base: ${base}">
                    <span class="stat-bar-tooltip">Base: ${base}</span>
                </div>`;
        }
        ...
        itensSelecionados.forEach((itemNome, idx) => {
            ...
            if (valorItem > 0) {
                ...
                segmentos += `
                    <div class="stat-bar-segment" style="width:${largura}%; background-color:${cor};" data-tooltip="${itemNome}: +${valorItem}">
                        <span class="stat-bar-tooltip">${itemNome}: +${valorItem}</span>
                    </div>`;
            }
        });
...
function iniciarTooltipsBarras() {
    document.addEventListener('mouseover', function(e) {
        ...
        if (!seg) return;
        const texto = seg.getAttribute('data-tooltip');
        let tooltip = seg.querySelector('.stat-bar-tooltip');
        if (!tooltip) {
            tooltip = document.createElement('span');
            tooltip.className = 'stat-bar-tooltip';
    ...
    document.addEventListener('mousemove', function(e) {
        const seg = e.target.closest('.stat-bar-segment');
        if (!seg) return;
        const tooltip = seg.querySelector('.stat-bar-tooltip');
        if (tooltip) {
            posicionarTooltip(e, tooltip);
        }
    });
...
// ============================================
// FUNÇÕES DE BARRAS EMPILHADAS E TOOLTIP
// ============================================
function posicionarTooltip(e, tooltip) {
    tooltip.style.left = e.clientX + 'px';
    tooltip.style.top = e.clientY + 'px';
